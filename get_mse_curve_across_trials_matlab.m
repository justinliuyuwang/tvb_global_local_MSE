function [mean_mse,std_mse,scales]= get_mse_curve_across_trials(data,m,r,scales)
...
% $$$   mean_mse = std mse across trials = vector of values, one value per scale
% $$$   scales = return vector of scales actually used by the program
  ...
  SampleEntropy = zeros(numel(scales),num_trials);
  ...
  for trial=1:num_trials
    ...
    for s=1:numel(scales)
      ...
      % this is wasteful because we could get sample entropy for all all m values [1:m], instead I am only calculating for actual m
      if (cont(m+1) == 0 | cont(m) == 0)
        SampleEntropy(s,trial) = -log(1/((nlin_sc)*(nlin_sc -1)));
      else
        SampleEntropy(s,trial) = -log(cont(m+1)/cont(m));
      end
  ...
  mean_mse = squeeze(mean(SampleEntropy,2)); % mean entropy across trials
  std_mse = squeeze(std(SampleEntropy,[],2)); % std entropy across trials
